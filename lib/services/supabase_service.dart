...
class SupabaseService {
        ...
        .map((List<Map<String, dynamic>> data) {
          return data.map((json) {
            ...
            final items = itemsJson.map((item) {
              ...
              return OrderItem(
                id: item['instanceId'], // Unique ID for this specific item instance
                menuItem: menuItem,
  ...
  Stream<List<CafeTable>> get tablesStream {
    return _client
        ...
        .map((data) => data.map((json) => CafeTable(
          id: json['id'],
          name: json['name'],
  ...
  Future<void> placeOrder(Order order) async {
    ...
    await _client.from('tables').update({'is_occupied': true}).eq('id', order.tableId);
  }
  ...
  Future<void> freeTable(String tableId) async {
     // Mark table free
    await _client.from('tables').update({'is_occupied': false}).eq('id', tableId);
  }
}
